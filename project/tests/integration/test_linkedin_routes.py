...
def test_linkedin_auth_start(mock_gen_state, client, app):
    ...
    app.redis_client.setex.assert_called_once_with(f"linkedin_state_{VALID_STATE}", 300, 'valid')
...
def test_linkedin_callback_invalid_state(client, app):
    app.redis_client.get = MagicMock(return_value=None) # Simulate state not found or expired
...
def test_linkedin_callback_token_exchange_fails(mock_requests_post, client, app):
    ...
    mock_token_response.json.return_value = {"error": "invalid_request", "error_description": "Auth code error"}
    mock_token_response.text = json.dumps(mock_token_response.json.return_value) # For error logging
    mock_requests_post.return_value = mock_token_response
    ...
    json_response = response.get_json()
    assert not json_response["success"]
    assert "Failed to exchange authorization code for access token." in json_response["error"]
    mock_requests_post.assert_called_once()

@patch('project.app.linkedin.routes.requests.post')
@patch('project.app.linkedin.routes.requests.get')
def test_linkedin_callback_userinfo_request_fails(mock_requests_get, mock_requests_post, client, app):
...
def test_linkedin_status_authenticated(client, app):
    ...
    app.redis_client.get = MagicMock(side_effect=[
...
def test_linkedin_logout_success(client, app):
    ...
    app.redis_client.delete.assert_has_calls(expected_calls, any_order=True) # Order doesn't matter
