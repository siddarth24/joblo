...
class CloudConvertClient:
    def __init__(self, api_key: str, sandbox: bool = False):
        ...
        try:
            ...
            logger.error(
                f"Failed to configure CloudConvert in CloudConvertClient: {e}",
                exc_info=True,
            )
    ...
    def convert_md_to_docx(self, input_md_path: str, output_docx_path: str):
        ...
        """
        logger.info(
            f"CloudConvertClient attempting to convert {input_md_path} to {output_docx_path}"
        )

        if not os.path.exists(input_md_path):
            logger.error(
                f"Input Markdown file not found by CloudConvertClient: {input_md_path}"
            )
            raise FileNotFoundError(
                f"Input Markdown file does not exist: {input_md_path}"
            )
        ...
        try:
            ...
            if (
                not import_task
                or not import_task.get("result")
                or not import_task["result"].get("form")
            ):
                logger.error(
                    f"Import task or its result/form not found in CloudConvert job {job_id}. Job details: {job}"
                )
                raise RuntimeError(
                    f"Could not find import task details in CloudConvert job {job_id}."
                )
            ...
            logger.info(
                f"Uploading {input_md_path} to CloudConvert for job {job_id}..."
            )
            ...
            if job.get("status") != "finished":
                logger.error(
                    f"CloudConvert job {job_id} did not finish successfully. Status: {job.get('status')}. Job: {job}"
                )
                raise RuntimeError(
                    f"CloudConvert job {job_id} failed. Status: {job.get('status')}"
                )
            ...
            if (
                not export_task
                or not export_task.get("result")
                or not export_task["result"].get("files")
            ):
                logger.error(
                    f"Export task not found or not finished, or no files in result for job {job_id}. Job: {job}"
                )
                raise RuntimeError(
                    f"Could not find successful export task details in CloudConvert job {job_id}."
                )
            ...
            logger.info(
                f"Downloading converted file from {download_url} for job {job_id}..."
            )
            ...
            logger.info(
                f"File downloaded successfully from CloudConvert to: {output_docx_path} for job {job_id}"
            )

        except requests.exceptions.RequestException as req_err:
            logger.error(
                f"HTTP request error during CloudConvertClient operation (job {job_id if 'job_id' in locals() else 'unknown'}): {req_err}",
                exc_info=True,
            )
            raise ConnectionError(f"CloudConvert API request failed: {req_err}")
        except Exception as e:
            logger.error(
                f"Error during CloudConvertClient operation (job {job_id if 'job_id' in locals() else 'unknown'}): {e}",
                exc_info=True,
            )
