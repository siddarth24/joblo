...
def mock_current_app():
    ...
    app.redis_client = (
        MagicMock()
    )  # Mock redis client if needed by services directly (not for this specific test)
...
@patch(
    "project.app.services.current_app"
)  # Mock current_app where it's used in services.py
...
def test_initiate_ats_analysis_success(
    ...
    mock_load_prompt,
    mock_async_generate_resume,
    mock_flask_app_context,
    mock_current_app_fixture,
    mock_celery_task,  # Renamed mock_current_app to avoid conflict
):
    """Test successful ATS analysis initiation."""
    # Arrange
    mock_flask_app_context.return_value = (
        mock_current_app_fixture  # Assign fixture to the patch target
    )
    mock_load_prompt.return_value = "Mocked ATS Prompt"
    mock_create_embedded_resume.return_value = (
        "[embedded_resume]Test CV Text[/embedded_resume]"
    )
    ...
    job_data_dict = {
        ...
        "description": "Develop awesome stuff.",
    }
...
def test_initiate_ats_analysis_no_openai_key(
    mock_flask_app_context, mock_current_app_fixture
):
...
@patch("project.app.services.current_app")
def test_initiate_ats_analysis_invalid_job_json(
    mock_flask_app_context, mock_current_app_fixture
):
...
@patch("project.app.services.current_app")
def test_initiate_ats_analysis_empty_cv_text(
    mock_flask_app_context, mock_current_app_fixture
):
...
@patch("project.app.services.current_app")
@patch(
    "project.app.services.load_prompt",
    side_effect=FileNotFoundError("Prompt not found oops"),
)
def test_initiate_ats_analysis_load_prompt_fails(
    mock_load_prompt_call, mock_flask_app_context, mock_current_app_fixture
):
    ...
    assert (
        "Prompt not found oops" in result["error"]
    )  # Check if original error is part of message
...
@patch(
    "project.app.services.secure_filename", side_effect=lambda x: x
)  # Mock secure_filename to return input
...
def test_initiate_docx_conversion_with_markdown_content(
    ...
    mock_async_convert_task,
    mock_flask_app_context,
    mock_current_app,
    mock_celery_task,  # Changed mock_current_app_fixture to mock_current_app
):
    ...
    mock_current_app.config["UPLOAD_FOLDER"] = (
        "/tmp/joblo_test_uploads"  # Ensure it's set for the test
    )

    # Act
    with patch(
        "flask.url_for", return_value=f"http://localhost/tasks/{mock_celery_task.id}"
    ):
        result = ProcessingService.initiate_docx_conversion(
            markdown_content, None, output_filename_base
        )
    ...
    assert re.search(
        expected_temp_md_filename_regex, os.path.basename(actual_temp_md_path)
    )
    ...
    assert args_task_call[1] == os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], expected_docx_filename
    )  # output_docx_path
...
def test_initiate_docx_conversion_with_input_path(
    ...
    mock_async_convert_task.delay.return_value = mock_celery_task
    mock_current_app.config["UPLOAD_FOLDER"] = (
        "/tmp/joblo_test_uploads"  # Ensure it's set
    )

    input_md_path = os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], "existing.md"
    )
    ...
    with patch(
        ...
        result = ProcessingService.initiate_docx_conversion(
            None, input_md_path, output_filename_base
        )
...
def test_initiate_docx_conversion_no_input(mock_flask_app_context, mock_current_app):
    ...
    assert result["status_code"] == 400
    assert (
        "Either markdownContent or inputMarkdownFilePath is required" in result["error"]
    )
...
def test_initiate_docx_conversion_unsafe_path(
    ...
    result = ProcessingService.initiate_docx_conversion(
        None, unsafe_path, "base_filename"
    )
    assert result["success"] is False
    assert result["status_code"] == 400
...
def test_initiate_docx_conversion_input_path_not_exists(
    ...
    non_existent_path = os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], "i_dont_exist.md"
    )
    mock_os_abspath.return_value = non_existent_path

    result = ProcessingService.initiate_docx_conversion(
        None, non_existent_path, "base_filename"
    )
    assert result["success"] is False
    assert result["status_code"] == 400
...
def test_initiate_docx_conversion_celery_dispatch_fails(
    ...
    result = ProcessingService.initiate_docx_conversion(
        None, input_md_path, "base_filename"
    )

    assert result["success"] is False
    assert result["status_code"] == 500
    assert (
        "An unexpected server error occurred in service" in result["error"]
    )  # Service catches generic exceptions
...
def test_initiate_resume_generation_success_full_workflow(
    ...
    mock_os_path_exists,
    mock_os_remove,
    mock_async_convert_md_to_docx_task_s,
    mock_save_markdown_to_file_task_s,
    mock_async_generate_resume_task_s,
    mock_celery_chain,
    ...
    mock_save_uploaded_file.side_effect = lambda fs, folder, name: os.path.join(
        folder, name
    )
    ...
    mock_allowed_file.assert_any_call(
        resume_file.filename
    )  # Called for resume and KB files
    ...
    mock_celery_chain.assert_called_once_with(
        mock_sig_gen, mock_sig_save, mock_sig_docx
    )
...
def test_initiate_resume_generation_triggers_scraping(
    ...
    with (
        patch(
            "project.app.services.save_uploaded_file",
            return_value="/tmp/test_uploads/resume.pdf",
        ) as mock_save,
        patch(
            "project.app.services.extract_text_and_links_from_file",
            return_value=("cv text", None),
        ) as mock_extract,
        patch("project.app.services.allowed_file", return_value=True) as mock_allow,
        patch(
            "flask.url_for",
            return_value=f"http://localhost/tasks/{mock_celery_task.id}",
        ) as mock_url,
    ):
...
)  # Mock os.path.exists for potential cleanup checks
...
def test_initiate_job_application_processing_scraping_path(
    ...
    mock_flask_app_context.return_value = mock_current_app
    mock_current_app.config["UPLOAD_FOLDER"] = "/tmp/joblo_test_uploads"
    mock_save_uploaded_file.return_value = os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], "saved_resume.pdf"
    )
    ...
    # Act
    result = ProcessingService.initiate_job_application_processing(
        job_url, job_description_form, mock_file_storage
    )
    ...
    assert result["status_code"] == 202
    assert result["scrape_task_id"] == mock_celery_task.id
    assert result["resume_path"] == os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], "saved_resume.pdf"
    )
...
@patch(
    "project.app.services.create_embedded_resume", return_value="[embedded_cv_direct]"
)
...
@patch(
    "project.app.services.secure_filename", side_effect=lambda x: x
)  # Used for outputFilenameBase generation
@patch("os.path.exists", return_value=True)
@patch("os.remove")
@patch("flask.url_for")
def test_initiate_job_application_processing_direct_ats_path(
    ...
    mock_flask_app_context.return_value = mock_current_app
    mock_current_app.config["UPLOAD_FOLDER"] = "/tmp/joblo_test_uploads"
    saved_resume_path = os.path.join(
        mock_current_app.config["UPLOAD_FOLDER"], "saved_resume_direct.pdf"
    )
...
@patch("project.app.services.current_app")
def test_initiate_job_application_processing_no_resume_file(
    mock_flask_app_context, mock_current_app
):
    mock_flask_app_context.return_value = mock_current_app
    result = ProcessingService.initiate_job_application_processing(
        "http://job.url", None, None
    )
...
def test_initiate_job_application_processing_invalid_resume_type(
    ...
    mock_flask_app_context.return_value = mock_current_app
    result = ProcessingService.initiate_job_application_processing(
        "http://job.url", None, mock_file_storage
    )
    assert result["success"] is False and result["status_code"] == 400
...
@patch("project.app.services.save_uploaded_file")
@patch(
    "project.app.services.extract_text_and_links_from_file", return_value(("", None))
...
def test_initiate_job_application_processing_empty_extracted_cv(
    ...
    with patch(
        "os.remove"
    ) as mock_os_rem:  # Mock os.remove for this specific test path
        result = ProcessingService.initiate_job_application_processing(
            "http://job.url", None, mock_file_storage
        )
        assert result["success"] is False and result["status_code"] == 400
        assert "Extracted text from resume is empty" in result["error"]
        mock_os_rem.assert_called_once_with(
            "/tmp/uploads/file.pdf"
        )  # Check cleanup of empty file
