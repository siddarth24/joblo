...
from project.app.clients.scraper_client import (
    ScraperClient,
    ValueError as ScraperValueError,
    ConnectionError as ScraperConnectionError,
)
...
def test_scrape_job_data_linkedin_url_success(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    ...
    mock_scrape_linkedin_job.assert_called_once_with(
        VALID_LINKEDIN_COLLECTION_URL, "a_groq_key"
    )
    mock_main_adaptive_scraper.assert_not_called()


def test_scrape_job_data_other_url_success(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    ...
    mock_main_adaptive_scraper.assert_called_once_with(
        VALID_OTHER_URL, "another_groq_key"
    )
...
def test_scrape_job_data_other_url_no_groq_key_logs_warning(
    ...
    for call_args in mock_logger.warning.call_args_list:
        if (
            "Groq API key not provided, but adaptive scraper might require it"
            in call_args[0][0]
        ):
            found_warning = True
            break
...
def test_scrape_job_data_invalid_url_type(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    client = ScraperClient()
    with pytest.raises(
        ScraperValueError, match="A valid URL must be provided for scraping."
    ):
...
def test_scrape_job_data_linkedin_scraper_fails(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    ...
    with pytest.raises(
        ...
        match=f"Failed to scrape job data from {VALID_LINKEDIN_URL} due to: LinkedIn scrape crashed",
    ):
...
def test_scrape_job_data_adaptive_scraper_fails(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    ...
    with pytest.raises(
        ScraperConnectionError,
        match=f"Failed to scrape job data from {VALID_OTHER_URL} due to: Adaptive scrape crashed",
    ):
...
def test_scrape_job_data_scraper_returns_no_data(
    mock_scrape_linkedin_job, mock_main_adaptive_scraper
):
    ...
    with pytest.raises(
        ScraperValueError,
        match=f"No job data could be retrieved from {VALID_LINKEDIN_URL}.",
    ):
        client.scrape_job_data(VALID_LINKEDIN_URL)
    mock_scrape_linkedin_job.assert_called_once_with(VALID_LINKEDIN_URL, None)
...
@patch.dict(
    sys.modules, {"linkedin_scraper": None, "adaptive_screenshot_scraper": None}
)
...
def test_scraper_client_sys_path_manipulation_effect():
    ...
    project_dir_test_perspective = os.path.dirname(
        project_tests_dir
    )  # Should be 'project'
    ...
    scraper_client_file_path = inspect.getfile(
        scraper_client_module
    )  # .../project/app/clients/scraper_client.py

    _sc_module_path = os.path.abspath(
        os.path.join(os.path.dirname(scraper_client_file_path), "..", "..")
    )  # project/
    _sc_workspace_root = os.path.abspath(
        os.path.join(_sc_module_path, os.pardir)
    )  # workspace_root

    assert (
        _sc_workspace_root in sys.path
