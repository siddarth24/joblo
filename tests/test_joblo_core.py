...
if _PROJECT_DIR not in sys.path: # If clients are in project/clients not project/app/clients
    sys.path.insert(0, _PROJECT_DIR) # Ensure `project.app.clients` can be found if clients live there
...
def test_prepare_prompt_without_rag_chunks():
    ...
    assert "Additional Candidate Data:\n\n\nFocus on Python skills." in prompt # Check spacing with empty RAG
...
def test_prepare_prompt_with_rag_chunks():
    ...
    rag_chunks = ["RAG chunk 1 about ML project A.", "RAG chunk 2 about ML technique B."]
    expected_rag_block = "RAG chunk 1 about ML project A.\n\nRAG chunk 2 about ML technique B."

    prompt = prepare_prompt(job_desc, embedded_cv, custom_prompt_text, relevant_chunks=rag_chunks)
...
@patch('joblo_core.PromptTemplate') # Mock PromptTemplate if its construction needs specific checks
def test_generate_resume_success(mock_prompt_template_class, mock_llm_chain_class, mock_chat_openai_class):
    ...
    # Act
    result = generate_resume(mock_api_key, prompt_text, model=model_name, temperature=temp, max_tokens=max_t, top_p=top_p_val)
    ...
    mock_prompt_template_class.assert_called_once_with(input_variables=["prompt"], template="{prompt}")
    mock_llm_chain_class.assert_called_once_with(llm=mock_llm_instance, prompt=mock_prompt_template_instance)
...
def test_generate_resume_openai_error(mock_chat_openai_class):
    ...
    with pytest.raises(ConnectionError, match="Error communicating with OpenAI API: OpenAI API is down"):
...
@patch("builtins.open", new_callable=mock_open, side_effect=IOError("Cannot write to disk"))
def test_save_resume_io_error(mock_file_open):
    with pytest.raises(IOError, match="Error saving generated resume: Cannot write to disk"):
...
def test_extract_resume_with_text_and_links(mock_extract_func):
    ...
    expected_combined_text = f"{extracted_cv_text}\n\nExtracted Hyperlinks:\n" + "\n".join(extracted_cv_links)
...
@patch('joblo_core.extract_text_and_links_from_file', side_effect=Exception("Extraction failed badly"))
def test_extract_resume_extraction_error(mock_extract_func):
    resume_p = "/path/to/corrupt.pdf"
    with pytest.raises(ValueError, match="Error extracting resume: Extraction failed badly"):
...
def test_convert_md_to_docx_success(
    mock_file_open, mock_requests_get, mock_requests_post,
    mock_cc_job_wait, mock_cc_job_create, mock_cc_configure
):
    ...
    mock_upload_form = {"url": "http://upload.example.com", "parameters": {"key": "value"}}
    mock_import_task_result = {"result": {"form": mock_upload_form}}
    mock_job_creation_response = {
        "id": "test_job_id",
        "tasks": [
            {"name": "import-my-file", "operation": "import/upload", "status": "waiting", **mock_import_task_result}
        ]
    }
    ...
    mock_export_file_info = {"filename": "output.docx", "url": "http://download.example.com/file.docx"}
    mock_job_wait_response = {
        "id": "test_job_id",
        "status": "finished",
        "tasks": [
            {"name": "import-my-file", "operation": "import/upload", "status": "finished"},
            {"name": "convert-my-file", "operation": "convert", "status": "finished"},
            {"name": "export-my-file", "operation": "export/url", "status": "finished", "result": {"files": [mock_export_file_info]}}
        ]
    }
    ...
    mock_requests_post.assert_called_once_with(mock_upload_form["url"], data=mock_upload_form["parameters"], files=ANY) # ANY from unittest.mock if needed, or check specific file handle
    ...
    assert any(call_args[0][0] == output_docx_path and call_args[0][1] == 'wb' for call_args in mock_file_open.call_args_list)
...
@patch('joblo_core.cloudconvert.configure')
@patch('joblo_core.cloudconvert.Job.create', side_effect=Exception("CloudConvert API error during Job.create"))
def test_convert_md_to_docx_job_create_fails(mock_cc_job_create, mock_cc_configure):
    with pytest.raises(RuntimeError, match="Error during CloudConvert conversion: CloudConvert API error during Job.create"):
        convert_md_to_docx("key", "in.md", "out.docx")
    mock_cc_configure.assert_called_once()


@patch('joblo_core.cloudconvert.configure')
@patch('joblo_core.cloudconvert.Job.create') # Returns a job, but one without an import task with form URL
@patch('joblo_core.requests.post') # Should not be called if import task is bad
def test_convert_md_to_docx_no_import_task_url(mock_requests_post, mock_cc_job_create, mock_cc_configure):
    mock_job_no_form = {
        "id": "job_no_form_id",
        "tasks": [
            {"name": "import-my-file", "operation": "import/upload", "status": "waiting", "result": {"form": None}} # No form URL
        ]
    }
    mock_cc_job_create.return_value = mock_job_no_form
    with pytest.raises(RuntimeError) as excinfo: # Actual error is likely TypeError or KeyError due to missing structure
        convert_md_to_docx("key", "in.md", "out.docx")
    # Check if the error message contains something about the form or parameters being None
    assert "'NoneType' object is not subscriptable" in str(excinfo.value) or "form" in str(excinfo.value).lower() 
...
@patch('joblo_core.cloudconvert.configure')
@patch('joblo_core.cloudconvert.Job.create')
@patch('joblo_core.requests.post', side_effect=requests.exceptions.RequestException("Upload failed"))
@patch('builtins.open', new_callable=mock_open)
def test_convert_md_to_docx_upload_fails(
    ...
    mock_import_task_result = {"result": {"form": mock_upload_form}}
    mock_cc_job_create.return_value = {"id": "job1", "tasks": [{"operation": "import/upload", **mock_import_task_result}]}

    with pytest.raises(ConnectionError, match="HTTP request error during CloudConvert conversion: Upload failed"):
...
@patch('joblo_core.cloudconvert.Job.wait', side_effect=Exception("Job wait failed unexpectedly"))
@patch('builtins.open', new_callable=mock_open)
def test_convert_md_to_docx_job_wait_fails(
    mock_file_open, mock_cc_job_wait, mock_requests_post, mock_cc_job_create, mock_cc_configure
):
    mock_api_key = "test_cc_key_wait_fail"
    # Setup mocks for successful job creation and upload
    mock_upload_form = {"url": "http://upload.example.com", "parameters": {}}
    mock_import_task_result = {"result": {"form": mock_upload_form}}
    mock_cc_job_create.return_value = {"id": "job_wait_fail", "tasks": [{"operation": "import/upload", **mock_import_task_result}]}
    mock_upload_response = MagicMock(); mock_upload_response.raise_for_status.return_value = None
    mock_requests_post.return_value = mock_upload_response

    with pytest.raises(RuntimeError, match="Error during CloudConvert conversion: Job wait failed unexpectedly"):
        convert_md_to_docx(mock_api_key, "in.md", "out.docx")
...
@patch('joblo_core.requests.post')
@patch('joblo_core.cloudconvert.Job.wait')
@patch('joblo_core.requests.get', side_effect=requests.exceptions.RequestException("Download failed"))
@patch('builtins.open', new_callable=mock_open)
def test_convert_md_to_docx_download_fails(
    mock_file_open, mock_requests_get, mock_cc_job_wait, mock_requests_post, mock_cc_job_create, mock_cc_configure
):
    mock_api_key = "cc_key_dl_fail"
    # Setup for successful path up to download
    mock_upload_form = {"url": "http://upload.example.com", "parameters": {}}
    mock_import_task_result = {"result": {"form": mock_upload_form}}
    mock_cc_job_create.return_value = {"id": "job_dl_fail", "tasks": [{"operation": "import/upload", **mock_import_task_result}]}
    mock_upload_response = MagicMock(); mock_upload_response.raise_for_status.return_value = None
    mock_requests_post.return_value = mock_upload_response
    mock_export_file_info = {"url": "http://download.example.com/failed.docx"}
    mock_job_wait_response_for_dl = {
        "id": "job_dl_fail", "status": "finished",
        "tasks": [{"name": "export-my-file", "operation": "export/url", "status": "finished", "result": {"files": [mock_export_file_info]}}]
    }
    mock_cc_job_wait.return_value = mock_job_wait_response_for_dl

    with pytest.raises(ConnectionError, match="HTTP request error during CloudConvert conversion: Download failed"):
        convert_md_to_docx(mock_api_key, "in.md", "out.docx")
    mock_requests_get.assert_called_once_with(mock_export_file_info["url"])
...
def test_convert_md_to_docx_output_write_fails(
    ...
    mock_upload_form = {"url": "http://upload.example.com", "parameters": {}}
    mock_import_task_result = {"result": {"form": mock_upload_form}}
    mock_cc_job_create.return_value = {"id": "job_write_fail", "tasks": [{"operation": "import/upload", **mock_import_task_result}]}
    mock_upload_response = MagicMock(); mock_upload_response.raise_for_status.return_value = None
    mock_requests_post.return_value = mock_upload_response
    mock_export_file_info = {"url": "http://download.example.com/doc.docx"}
    mock_job_wait_response_for_write = {
        "id": "job_write_fail", "status": "finished",
        "tasks": [{"name": "export-my-file", "operation": "export/url", "status": "finished", "result": {"files": [mock_export_file_info]}}]
    }
    mock_cc_job_wait.return_value = mock_job_wait_response_for_write
    mock_download_content = MagicMock(); mock_download_content.raise_for_status.return_value = None; mock_download_content.content = b"data"
    ...
    def open_side_effect(path, mode):
        ...
        return mock_open.return_value # Default behavior for other opens (e.g., input MD)
    mock_file_open.side_effect = open_side_effect
    
    with pytest.raises(RuntimeError, match="Error during CloudConvert conversion: Cannot write output DOCX"):
        convert_md_to_docx(mock_api_key, "in.md", "out.docx")
...
@patch('builtins.open', new_callable=mock_open) # For fallback prompt loading in run_joblo
def test_run_joblo_basic_flow_with_clients(
    mock_prompt_open, mock_makedirs, mock_os_path_exists,
    mock_core_convert_md_to_docx, mock_core_save_resume, mock_core_generate_resume, mock_core_adaptive_scraper,
    mock_prepare_prompt, mock_extract_relevant_chunks, mock_create_embedded_resume, mock_extract_resume,
    mock_scraper_client_fixture, mock_openai_client_fixture, mock_cc_client_fixture # Use client fixtures
):
    ...
    mock_core_adaptive_scraper.assert_called_once_with(mock_scraper_client_fixture, job_url)
    ...
    mock_core_generate_resume.assert_called_once_with(mock_openai_client_fixture, mock_final_llm_prompt_text)
    ...
    mock_core_convert_md_to_docx.assert_called_once_with(mock_cc_client_fixture, ANY, ANY)
...
def test_run_joblo_missing_clients():
    with pytest.raises(ValueError, match="run_joblo requires the following client instances: ScraperClient, OpenAIClient, CloudConvertClient"):
        run_joblo(None, None, None, "url", "resume.pdf")
    with pytest.raises(ValueError, match="run_joblo requires the following client instances: OpenAIClient, CloudConvertClient"):
        run_joblo(MagicMock(spec=ScraperClient), None, None, "url", "resume.pdf")
    with pytest.raises(ValueError, match="run_joblo requires the following client instances: CloudConvertClient"):
        run_joblo(MagicMock(spec=ScraperClient), MagicMock(spec=OpenAIClient), None, "url", "resume.pdf")
...
def test_run_joblo_uses_fallback_prompt_if_custom_not_provided(
    mock_makedirs, mock_save_resume, mock_prepare_prompt_in_fallback_test, # Renamed to avoid clash
    mock_extract_relevant_chunks, mock_create_embedded_resume, mock_extract_resume,
    mock_core_convert_md_to_docx, mock_core_generate_resume, mock_core_adaptive_scraper,
    mock_open_for_fallback_prompt, mock_os_path_exists_for_fallback, # Renamed for clarity
    mock_scraper_client_fixture, mock_openai_client_fixture, mock_cc_client_fixture
):
    ...
    mock_core_convert_md_to_docx.assert_called_once_with(mock_cc_client_fixture, ANY, ANY)
    assert mock_core_convert_md_to_docx.call_args[0][1].endswith("Fallback_joblo_generated.md")
    assert mock_core_convert_md_to_docx.call_args[0][2].endswith("Fallback_joblo_generated.docx")
    ...
    mock_core_generate_resume.assert_called_once_with(mock_openai_client_fixture, "Final prompt for LLM, for testing run_joblo.")
    mock_core_adaptive_scraper.assert_called_once_with(mock_scraper_client_fixture, "some_url_for_fallback")
