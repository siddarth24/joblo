...
def check_redis_client():
    if not current_app.redis_client:
        logger.error("Redis client is not available. LinkedIn state operations will fail.")
        return False
    return True
...
def store_state():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Failed to save state due to Redis error: {str(e_redis)}"}), 500
    except Exception as e:
        logger.error(f"Failed to save state for key {redis_key}: {str(e)}")
        return jsonify({"success": False, "error": f"Failed to save state: {str(e)}"}), 500
...
def retrieve_state(unique_id: str):
    ...
    try:
        ...
        if state_data_json:
            state_data = json.loads(state_data_json) # Assuming state_data_json is a JSON string
        ...
        logger.error(f"Redis error retrieving state for key {redis_key}: {str(e_redis)}")
        return jsonify({"success": False, "error": f"Failed to retrieve state due to Redis error: {str(e_redis)}"}), 500
    except json.JSONDecodeError as e_json:
        logger.error(f"Failed to parse state JSON from Redis for key {redis_key}: {str(e_json)}")
        # Potentially delete the malformed key or log it for investigation
        return jsonify({"success": False, "error": "Failed to parse stored state data."}), 500
    except Exception as e:
        logger.error(f"Failed to read state for key {redis_key}: {str(e)}")
        return jsonify({"success": False, "error": f"Failed to read state: {str(e)}"}), 500
...
def delete_state(unique_id: str):
    ...
    try:
        ...
        if deleted_count > 0:
            ...
            logger.warning(f"State not found in Redis for key {redis_key} during delete attempt.")
            return jsonify({"success": False, "error": "State not found."}), 404
    except redis.exceptions.RedisError as e_redis:
        logger.error(f"Redis error deleting state for key {redis_key}: {str(e_redis)}")
        return jsonify({"success": False, "error": f"Failed to delete state due to Redis error: {str(e_redis)}"}), 500
    except Exception as e:
        logger.error(f"Failed to delete state for key {redis_key}: {str(e)}")
        return jsonify({"success": False, "error": f"Failed to delete state: {str(e)}"}), 500
...
def authenticate():
    ...
    try:
        if current_app.redis_client.exists(redis_key):
            logger.info(f"Authentication successful: State found in Redis for unique_id: {unique_id}")
            return jsonify({
                "success": True,
            ...
            logger.warning(f"Authentication failed: State not found in Redis for unique_id: {unique_id}")
            return jsonify({
                "success": False,
                "error": "LinkedIn session state not found or expired. Please log in again via the extension."
            }), 404 # Or 401 Unauthorized
    except redis.exceptions.RedisError as e_redis:
        logger.error(f"Redis error during authentication for unique_id {unique_id}: {str(e_redis)}")
        return jsonify({"success": False, "error": "Authentication failed due to a server-side Redis error."}), 500
    except Exception as e:
        logger.error(f"Error during authentication for unique_id {unique_id}: {str(e)}")
        return jsonify({"success": False, "error": "Authentication failed due to an unexpected server error."}), 500 