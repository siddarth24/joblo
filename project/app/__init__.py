...
from ..celery_app import create_celery_app # Import the Celery app factory
...
def create_app(config_class=Config):
    ...
    # Initialize Celery
    app.celery_app = create_celery_app(app)
    ...
    try:
        app.redis_client = redis.StrictRedis(
            ...
            password=app.config.get('REDIS_PASSWORD'), # Will be None if not set, which is fine for no-auth Redis
        ...
        app.logger.error(f"An unexpected error occurred during Redis initialization: {e_redis_other}", exc_info=True)
    ...
    try:
        ...
        app.logger.error(f"OpenAIClient initialization failed: Missing config key {e_key}", exc_info=True)
        app.openai_client = None # Explicitly set to None on failure
    except Exception as e_openai:
        app.logger.error(f"OpenAIClient initialization failed: {e_openai}", exc_info=True)
    ...
    try:
        ...
        app.cloudconvert_client = CloudConvertClient(
            api_key=app.config['CLOUDCONVERT_API_KEY'],
            sandbox=sandbox
        )
        app.logger.info("CloudConvertClient initialized.")
    except KeyError as e_key:
        app.logger.error(f"CloudConvertClient initialization failed: Missing config key {e_key}", exc_info=True)
        app.cloudconvert_client = None
    except Exception as e_cc:
        app.logger.error(f"CloudConvertClient initialization failed: {e_cc}", exc_info=True)
        app.cloudconvert_client = None
    ...
    try:
        ...
        app.scraper_client = ScraperClient(
            groq_api_key=app.config.get('GROQ_API_KEY') # .get() allows it to be None if not set
        )
        app.logger.info("ScraperClient initialized.")
    except Exception as e_scraper: # ScraperClient import itself might fail if its deps fail
        app.logger.error(f"ScraperClient initialization failed: {e_scraper}", exc_info=True)
    ...
    for folder_key in ['UPLOAD_FOLDER', 'STATE_FOLDER', 'LOG_DIR']:
        ...
        elif not folder_path and folder_key == 'LOG_DIR': # Ensure LOG_DIR has a default if not set
    ...
    if not cors_origins and cors_origins != "*": # Check if empty list and not wildcard
        app.logger.warning("CORS_ORIGINS is not configured or empty, CORS might be restrictive or disabled depending on environment.")
    ...
    CORS(app, resources={r"/*": {"origins": cors_origins, "allow_headers": ["Content-Type", "Authorization"], "supports_credentials": True}})
    ...
    ensure_directories_exist(app) # ensure_directories_exist should use app.config paths
    ...
    if os.environ.get("LANGCHAIN_API_KEY") and os.environ.get("LANGCHAIN_TRACING_V2", "false").lower() == "true":
        app.logger.info("Langchain Tracing is expected to be active based on environment variables.")
    else:
        app.logger.info("Langchain Tracing is not expected to be active or LANGCHAIN_API_KEY is missing.")
    ...
    app.logger.info(f"Joblo API configured. UPLOAD_FOLDER: {app.config.get('UPLOAD_FOLDER')}, STATE_FOLDER: {app.config.get('STATE_FOLDER')}, LOG_DIR: {app.config.get('LOG_DIR')}")
