...
from project.tasks import (
    ...
    Ignore,
)  # Import Ignore for testing specific error handling
...
@pytest.fixture
def mock_flask_current_app_for_tasks(
    mock_openai_client, mock_cloudconvert_client, mock_scraper_client
):
...
def test_async_generate_resume_cache_miss_then_hit(
    ...
    cache_key = (
        f"llm_gen:{prompt},{expected_model},{expected_temp}"  # Simplified for example
    )
    ...
    # --- Act for Cache Miss ---
    result_miss = async_generate_resume(mock_celery_task_self, prompt)
    ...
    mock_core_gen_resume.assert_called_once_with(
        mock_flask_current_app_for_tasks.openai_client, prompt
    )
    ...
    # --- Act for Cache Hit ---
    result_hit = async_generate_resume(mock_celery_task_self, prompt)
...
def test_async_generate_resume_caching_disabled(
    ...
    mock_core_gen_resume,
    mock_flask_app_ctx_for_task,
    mock_flask_current_app_for_tasks,
...
def test_async_generate_resume_redis_get_error(
    ...
    mock_gen_cache_key.assert_called_once_with(
        "llm_resume_gen", prompt, **client_params
    )
    mock_redis.get.assert_called_once_with(cache_key)
    mock_core_gen_resume.assert_called_once_with(
        mock_flask_current_app_for_tasks.openai_client, prompt
...
def test_async_generate_resume_redis_setex_error(
    ...
    mock_redis.setex.assert_called_once_with(
        cache_key, 3600, "LLM output for setex fail"
    )
...
def test_async_generate_resume_openai_client_not_initialized(
    ...
    mock_flask_current_app_for_tasks.openai_client = (
        None  # Simulate client not initialized
    )
    mock_flask_app_ctx_for_task.return_value = mock_flask_current_app_for_tasks
    prompt = "Test prompt"
...
def test_async_adaptive_scraper_cache_miss_then_hit(
    ...
    mock_core_scraper.assert_called_once_with(
        mock_flask_current_app_for_tasks.scraper_client, url
    )
    mock_redis.setex.assert_called_once_with(cache_key, 1800, scraped_data_json_str)

    mock_redis.get.reset_mock()
    mock_core_scraper.reset_mock()
    mock_redis.setex.reset_mock()
    mock_gen_cache_key.reset_mock()
...
def test_async_adaptive_scraper_redis_setex_error(
    ...
    mock_redis.setex.assert_called_once_with(
        cache_key, 1800, json.dumps(scraped_output)
    )
...
def test_async_convert_md_to_docx_delete_input_success(
    ...
    # Act
    result = async_convert_md_to_docx(
        mock_celery_task_self, input_path, output_path, delete_input_on_success=True
    )
    ...
    mock_core_convert.assert_called_once_with(
        mock_flask_current_app_for_tasks.cloudconvert_client, input_path, output_path
    )
...
def test_async_convert_md_to_docx_delete_input_false(
    ...
    result = async_convert_md_to_docx(
        mock_celery_task_self, input_path, output_path, delete_input_on_success=False
    )
...
@patch("project.tasks.core_convert_md_to_docx")
@patch(
    "project.tasks.os.path.exists", return_value=True
)  # File exists for deletion attempt
...
def test_async_convert_md_to_docx_core_function_raises_file_not_found(
    ...
    mock_core_convert.side_effect = FileNotFoundError(
        f"Input Markdown file does not exist: {input_path}"
    )

    with pytest.raises(
        FileNotFoundError, match=f"Input Markdown file does not exist: {input_path}"
    ):
        async_convert_md_to_docx(
            mock_celery_task_self, input_path, output_path, delete_input_on_success=True
        )
...
def test_save_markdown_to_file_task_filename_sanitization_and_md_extension(
    ...
    mock_open_file.assert_called_with(
        os.path.join(upload_dir, expected_final_filename_1), "w", encoding="utf-8"
    )

    mock_basename.reset_mock()
    ...
    mock_open_file.assert_called_with(
        os.path.join(upload_dir, expected_final_filename_2), "w", encoding="utf-8"
    )
...
def test_save_markdown_to_file_task_upload_folder_creation_error(
    ...
    expected_error_msg = (
        f"UPLOAD_FOLDER {upload_dir} does not exist and could not be created."
    )
