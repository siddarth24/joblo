...
from project.app.clients.cloudconvert_client import (
    CloudConvertClient,
    ConnectionError as CloudConvertClientConnectionError,
    FileNotFoundError,
    RuntimeError as CloudConvertClientRuntimeError,
)
...
def valid_cloudconvert_client_config():
    return {"api_key": "test_cc_key", "sandbox": True}
...
def test_cloudconvert_client_initialization_success(
...
def test_cloudconvert_client_initialization_failure(
    valid_cloudconvert_client_config, mock_cloudconvert_lib
):
    mock_cloudconvert_lib.configure.side_effect = Exception("Config failed")
    with pytest.raises(
        CloudConvertClientConnectionError,
        match="CloudConvertClient configuration failed: Config failed",
    ):
...
def mock_job_create_success(mock_cloudconvert_lib):
    ...
    mock_job_create_response = {
        "id": mock_job_id,
        "tasks": [{"name": "import-my-file", "result": {"form": mock_upload_form}}],
    }
...
def test_convert_md_to_docx_success(
    valid_cloudconvert_client_config,
    mock_cloudconvert_lib,
    mock_job_create_success,
    mock_upload_success,
    mock_job_wait_success,
    mock_download_success,
    mock_os_path_exists,
    mock_file_system,  # mock_file_system for open calls
):
    ...
    mock_requests_post.assert_called_once_with(
        upload_form["url"], data=upload_form["parameters"], files=ANY
    )
...
def test_convert_md_to_docx_input_file_not_found(
    valid_cloudconvert_client_config, mock_os_path_exists
):
    mock_os_path_exists.return_value = False
    client = CloudConvertClient(**valid_cloudconvert_client_config)
    with pytest.raises(
        FileNotFoundError, match=f"Input Markdown file does not exist: {INPUT_MD_PATH}"
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
    mock_os_path_exists.assert_called_once_with(INPUT_MD_PATH)
...
def test_convert_md_to_docx_job_create_fails(
    mock_logger,
    valid_cloudconvert_client_config,
    mock_cloudconvert_lib,
    mock_os_path_exists,
    ...
    client = CloudConvertClient(**valid_cloudconvert_client_config)
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match="General error during CloudConvert conversion: Create job API error",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)


@patch("project.app.clients.cloudconvert_client.logger")
def test_convert_md_to_docx_import_task_details_missing(
    mock_logger,
    valid_cloudconvert_client_config,
    mock_cloudconvert_lib,
    mock_os_path_exists,
    ...
    mock_os_path_exists.return_value = True
    mock_cloudconvert_lib.Job.create.return_value = {
        "id": "job_no_import_form",
        "tasks": [{"name": "import-my-file", "result": None}],
    }  # Missing form
    client = CloudConvertClient(**valid_cloudconvert_client_config)
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match="Could not find import task details in CloudConvert job job_no_import_form",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
...
def test_convert_md_to_docx_upload_fails_http_error(
    ...
    with pytest.raises(
        CloudConvertClientConnectionError,
        match="CloudConvert API request failed: Upload failed with 403",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
...
def test_convert_md_to_docx_job_wait_fails_or_not_finished(
    ...
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match="General error during CloudConvert conversion: Wait API error",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
    ...
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match=f"CloudConvert job {job_id} failed. Status: error",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
...
def test_convert_md_to_docx_export_task_details_missing(
    ...
    mock_cloudconvert_lib.Job.wait.return_value = {
        ...
        "status": "finished",
        "tasks": [
            {"name": "export-my-file", "status": "finished", "result": None}
        ],  # Missing files in result
    }
    client = CloudConvertClient(**valid_cloudconvert_client_config)
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match=f"Could not find successful export task details in CloudConvert job {job_id}",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
...
def test_convert_md_to_docx_download_fails_http_error(
    ...
    with pytest.raises(
        CloudConvertClientConnectionError,
        match="CloudConvert API request failed: Download network error",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
...
def test_convert_md_to_docx_write_output_fails(
    ...
    with pytest.raises(
        CloudConvertClientRuntimeError,
        match="General error during CloudConvert conversion: Cannot write to output file",
    ):
        client.convert_md_to_docx(INPUT_MD_PATH, OUTPUT_DOCX_PATH)
