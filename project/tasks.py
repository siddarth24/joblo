...
DEFAULT_RETRY_POLICY = {
    'autoretry_for': (requests.exceptions.RequestException, ConnectionError), # General network issues + client ConnectionErrors
...
def async_generate_resume(self, prompt_text: str):
    ...
    logger.info(f"Task async_generate_resume started. ID: {self.request.id}. Prompt (start): {prompt_text[:50]}...")

    openai_client = flask_current_app.openai_client
    if not openai_client:
        logger.error(f"OpenAIClient not initialized in Flask app. Cannot run async_generate_resume. Task ID: {self.request.id}")
    ...
    if cache_enabled and redis_client:
        try:
            ...
            logger.error(f"Redis GET error for key {cache_key} (ID: {self.request.id}): {e_cache_get}", exc_info=True)
    ...
    try:
        ...
        logger.info(f"Task async_generate_resume completed (core_generate_resume call). ID: {self.request.id}")

        if cache_enabled and redis_client and cache_key and generated_content is not None:
            try:
                ttl = flask_current_app.config.get('LLM_CACHE_TTL_SECONDS', 86400)
                redis_client.setex(cache_key, ttl, generated_content)
                logger.info(f"Stored result in cache for key: {cache_key} with TTL: {ttl}s. ID: {self.request.id}")
            except Exception as e_cache_set:
                logger.error(f"Redis SETEX error for key {cache_key} (ID: {self.request.id}): {e_cache_set}", exc_info=True)

        return generated_content
    except ConnectionError as e_client_conn:
        logger.error(f"OpenAIClient ConnectionError in async_generate_resume (ID: {self.request.id}): {e_client_conn}", exc_info=True)
        raise # Re-raise for Celery to handle (retry based on policy)
    except ValueError as e_client_val:
        logger.error(f"OpenAIClient ValueError in async_generate_resume (ID: {self.request.id}): {e_client_val}", exc_info=True)
        ...
        logger.error(f"Generic error in async_generate_resume (ID: {self.request.id}): {e}", exc_info=True)
        raise
...
def async_convert_md_to_docx(self, input_md_path: str, output_docx_path: str, delete_input_on_success: bool = False):
    logger.info(f"Task async_convert_md_to_docx started for {input_md_path}. ID: {self.request.id}. Delete input: {delete_input_on_success}")
    ...
    try:
        ...
        if delete_input_on_success:
            try:
                if os.path.exists(input_md_path):
                    ...
                    logger.info(f"Successfully deleted input MD file: {input_md_path}. ID: {self.request.id}")
                else:
                    logger.warning(f"Input MD file {input_md_path} not found for deletion. ID: {self.request.id}")
            except Exception as e_delete:
                logger.error(f"Failed to delete input MD file {input_md_path} (ID: {self.request.id}): {e_delete}", exc_info=True)

        return output_docx_path
    except (FileNotFoundError, ConnectionError, RuntimeError) as e_client: # Errors from CloudConvertClient
        logger.error(f"CloudConvertClient error in async_convert_md_to_docx (ID: {self.request.id}): {e_client}", exc_info=True)
        ...
        logger.error(f"Generic error in async_convert_md_to_docx (ID: {self.request.id}): {e}", exc_info=True)
        raise
        ...
        try:
            ...
            if redis_client:
                ...
                if cached_result_str:
                    logger.info(f"Cache HIT for scraper key: {cache_key}. ID: {self.request.id}")
                    try:
                        job_data = json.loads(cached_result_str.decode('utf-8'))
                        return job_data
                    except json.JSONDecodeError as e_json:
                        logger.error(f"Failed to decode cached JSON for key {cache_key}: {e_json}. Proceeding to scrape. ID: {self.request.id}")
                else:
                    logger.info(f"Cache MISS for scraper key: {cache_key}. ID: {self.request.id}")
            else: 
                logger.warning(f"Redis client became unavailable before GET. Key: {cache_key}. ID: {self.request.id}")
        except Exception as e_cache_get:
            logger.error(f"Redis GET error for scraper key {cache_key} (ID: {self.request.id}): {e_cache_get}", exc_info=True)

    try:
        # API key (Groq) is handled by the client.
        job_data = core_adaptive_scraper(scraper_client, url)
        logger.info(f"Task async_adaptive_scraper completed (core_adaptive_scraper call) for URL: {url}. ID: {self.request.id}")
        ...
        if cache_enabled and redis_client and cache_key and job_data is not None:
            try:
                ...
                logger.info(f"Stored scraper result in cache for key: {cache_key}, TTL: {ttl}s. ID: {self.request.id}")
            except TypeError as e_type_json:
                 logger.error(f"Failed to serialize job_data to JSON for key {cache_key}: {e_type_json}. Type: {type(job_data)}. ID: {self.request.id}")
            except Exception as e_cache_set:
                logger.error(f"Redis SETEX error for scraper key {cache_key} (ID: {self.request.id}): {e_cache_set}", exc_info=True)
        
        return job_data
    except (ValueError, ConnectionError) as e_client: # Errors from ScraperClient
        logger.error(f"ScraperClient error in async_adaptive_scraper for URL {url} (ID: {self.request.id}): {e_client}", exc_info=True)
        if isinstance(e_client, ConnectionError):
            raise # Let Celery retry
        raise # For ValueError or others, don't retry by default via this policy
    except Exception as e:
        logger.error(f"Generic error in async_adaptive_scraper for URL {url} (ID: {self.request.id}): {e}", exc_info=True)
        raise
