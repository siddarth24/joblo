...
def process_job_application():
    """Initiates asynchronous job application processing (scraping & initial ATS) via the ProcessingService."""
    try:
        job_url = request.form.get('jobUrl')
        job_description_form = request.form.get('jobDescription')
        resume_file_storage = request.files.get('resumeFile') # werkzeug.datastructures.FileStorage object

        # Basic validation (more detailed validation is in the service)
        if not resume_file_storage:
            return jsonify({"success": False, "error": "Resume file is required."}), 400
        if not (job_url or job_description_form):
        ...
        result = ProcessingService.initiate_job_application_processing(
            job_url=job_url,
            job_description_form=job_description_form,
        ...
        logger.error(f"Unexpected error in /process-job-application route: {str(e)}", exc_info=True)
...
def generate_resume_endpoint():
    ...
    try:
        ...
        resume_file_storage = request.files.get('resumeFile') # werkzeug.datastructures.FileStorage object
        kb_file_storages = request.files.getlist('kbFiles')   # list of FileStorage objects
        ...
        logger.error(f"Unexpected error in /generate-resume route: {str(e)}", exc_info=True)
...
def convert_to_docx_endpoint():
    ...
    try:
        ...
        logger.error(f"Unexpected error in /convert-to-docx route: {str(e)}", exc_info=True)
        return jsonify({"success": False, "error": f"An unexpected server error occurred in route: {str(e)}"}), 500
