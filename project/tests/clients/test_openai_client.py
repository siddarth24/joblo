...
from project.app.clients.openai_client import OpenAIClient, ConnectionError as OpenAIClientConnectionError # Alias to avoid clash with built-in
...
def test_openai_client_initialization_success(
    ...
    mock_llm_chain_class.assert_called_once_with(llm=mock_llm_instance, prompt=mock_pt_instance)
...
def test_openai_client_initialization_chatopenai_fails(
    ...
    with pytest.raises(OpenAIClientConnectionError, match="OpenAIClient initialization failed: ChatOpenAI init failed"):
...
def test_openai_client_initialization_llmchain_fails(
    ...
    with pytest.raises(OpenAIClientConnectionError, match="OpenAIClient initialization failed: LLMChain init failed"):
        OpenAIClient(**valid_openai_client_config)
...
def test_generate_text_chain_not_initialized(valid_openai_client_config, mock_chat_openai_class):
...
def test_generate_text_llm_chain_run_fails(
    ...
    with pytest.raises(OpenAIClientConnectionError, match="Error communicating with OpenAI API via OpenAIClient: LLM run failed"):
...
def test_openai_client_logging_on_init_and_generation(
    ...
    with patch.object(actual_logger, 'info') as mock_logger_info, \
        ...
        client = OpenAIClient(**valid_openai_client_config)
        mock_logger_info.assert_any_call(f"OpenAIClient initialized successfully for model: {client.model_name}")

        client.generate_text("test prompt")
        mock_logger_debug.assert_any_call(f"OpenAIClient generating text for prompt (first 50 chars): test prompt...")
        mock_logger_info.assert_any_call(f"OpenAIClient successfully generated text (length: 4).")

    # Re-disable for other tests
    actual_logger.setLevel(logging.CRITICAL)


def test_openai_client_init_connection_error_on_chat_open_ai_exception(valid_openai_client_config, mock_chat_openai_class):
    ...
    with pytest.raises(OpenAIClientConnectionError, match="OpenAIClient initialization failed: Underlying library error"):
        OpenAIClient(**valid_openai_client_config)

def test_generate_text_connection_error_on_chain_run_exception(
    valid_openai_client_config, mock_chat_openai_class, mock_llm_chain_class, mock_prompt_template_class
):
    ...
    client = OpenAIClient(**valid_openai_client_config)
    with pytest.raises(OpenAIClientConnectionError, match="Error communicating with OpenAI API via OpenAIClient: Underlying chain.run error"):
