...
def create_app(config_class=Config):
    ...
    if app.config.get('REDIS_URL'):
        ...
        app.logger.info(f"Redis client initialized from REDIS_URL.")
    else:
        try:
            app.redis_client = redis.StrictRedis(
                ...
                password=app.config.get('REDIS_PASSWORD'), # password can be None
                decode_responses=True # Important for string operations
            )
            app.redis_client.ping() # Verify connection
            app.logger.info(f"Redis client initialized with host: {app.config['REDIS_HOST']}:{app.config['REDIS_PORT']}, db: {app.config['REDIS_DB']}.")
        except redis.exceptions.ConnectionError as e_redis:
            app.logger.error(f"Failed to connect to Redis: {e_redis}. LinkedIn state management will not work.")
            app.redis_client = None # Set to None if connection fails
        except Exception as e_redis_other:
            app.logger.error(f"An unexpected error occurred during Redis initialization: {e_redis_other}")
