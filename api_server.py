...
def process_job_application():
    ...
    try:
        ...
        if job_url:
                 job_data = {"Job Title": "Job from URL (Scraping Failed)", "Company": "Unknown", "Description": f"Could not scrape job from {job_url} due to missing GROQ_API_KEY.", "SourceURL": job_url}
        elif job_description:
                 job_data = {"Job Title": "Job from Text", "Company": "Company from Text", "Description": job_description}
        else:
                return jsonify({"success": False, "error": "GROQ_API_KEY missing and no job data fallback possible."}), 500
        
        if job_url and GROQ_API_KEY:
            logger.info(f"Attempting to scrape job from URL: {job_url} using adaptive_scraper")
            try:
                job_data = adaptive_scraper(job_url, GROQ_API_KEY)
                if not job_data or not isinstance(job_data, dict):
                    logger.warning(f"Adaptive scraper returned empty or invalid data for {job_url}. Falling back to placeholder.")
                    job_data = {"Job Title": "Job from URL (Scraping Issue)", "Company": "Unknown", "Description": f"Issue scraping job from {job_url}. Source URL: {job_url}", "SourceURL": job_url}
            except Exception as scrape_exc:
                logger.error(f"Error during adaptive_scraper call for {job_url}: {scrape_exc}")
                job_data = {"Job Title": "Job from URL (Scraping Error)", "Company": "Unknown", "Description": f"Error scraping job from {job_url}. Error: {scrape_exc}. Source URL: {job_url}", "SourceURL": job_url}
        elif job_description: # Only job description text is provided
            logger.info("Using provided job description text directly")
            job_data = {"Job Title": "Job from Text", "Company": "Company from Text", "Description": job_description}
        elif job_url and not GROQ_API_KEY: # GROQ key missing, job_data already has a placeholder from above
            logger.warning(f"Proceeding with placeholder job data for {job_url} due to missing GROQ_API_KEY.")
            ...
            return jsonify({"success": False, "error": "No job URL or description provided for processing."}), 400
        ...
        if "Description" not in job_data or not job_data["Description"]:
            ...
            elif job_url: # If only URL was provided and scraping failed to get description
                job_data["Description"] = f"Job details from {job_url} (Description not extracted)."
            else:
                job_data["Description"] = "No job description available."

        # Corrected usage of the imported function
        try:
            cv_text, _ = extract_text_and_links_from_file(resume_path) # We only need the text for now
            logger.info("Successfully extracted text from resume")
        except FileNotFoundError:
            logger.error(f"Resume file not found at path: {resume_path} during extraction.")
            return jsonify({"success": False, "error": f"Resume file not found at {resume_path}."}), 500
        except Exception as resume_exc:
            logger.error(f"Error extracting text from resume {resume_path}: {resume_exc}")
            return jsonify({"success": False, "error": f"Failed to extract text from resume: {resume_exc}"}), 500
        ...
        if not cv_text or not cv_text.strip():
            ...
            return jsonify({"success": False, "error": "Extracted text from resume is empty. Please check the resume file."}), 400
        ...
        ats_output_str = gpt_generate_resume(openai_api_key, prompt_ats, model="gpt-4o-mini", temperature=0.1) # Lowered temperature
        
        import re
        json_match_ats = re.search(r'```json\\s*(.*?)\\s*```|{.*}', ats_output_str, re.DOTALL)
        
        if json_match_ats:
            json_str_ats = json_match_ats.group(1) or json_match_ats.group(0)
            json_str_ats = json_str_ats.replace('```json', '').replace('```', '').strip()
            try:
                ats_score = json.loads(json_str_ats)
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse ATS JSON from LLM output after regex: {json_str_ats}. Error: {e}")
                return jsonify({"success": False, "error": "Failed to parse ATS score JSON. Invalid format from AI."}), 500
        else:
            logger.error(f"Failed to find ATS JSON in LLM output: {ats_output_str}")
            return jsonify({"success": False, "error": "Failed to find ATS score JSON in AI response."}), 500
        ...
        logger.info(f"Initial job application processed successfully for resume: {resume_file.filename}")
...
def analyze_ats():
    ...
    try:
        if 'jobData' not in request.form or 'cvText' not in request.form:
            ...
            return jsonify({"success": False, "error": "Missing job data or CV text."}), 400
        ...
        ats_output_str = gpt_generate_resume(openai_api_key, prompt, model="gpt-4o-mini", temperature=0.1) # Lowered temperature
        
        import re
        json_match = re.search(r'```json\\s*(.*?)\\s*```|{.*}', ats_output_str, re.DOTALL)
        ats_score_data = {}
        ...
        if json_match:
            ...
            try:
                ...
                logger.error(f"Failed to parse ATS JSON from LLM output after regex: {json_str}. Error: {e}")
                return jsonify({"success": False, "error": "Failed to parse ATS score JSON. Invalid format from AI."}), 500
        else:
            logger.error(f"Failed to find ATS JSON in LLM output: {ats_output_str}")
            return jsonify({"success": False, "error": "Failed to find ATS score JSON in AI response."}), 500
        ...
        return jsonify({
            "success": True,
            "data": {"atsScore": ats_score_data}
        })
            
    except Exception as e:
        logger.error(f"Error during ATS analysis: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({"success": False, "error": f"Failed to analyze ATS score: {str(e)}"}), 500
...
def generate_resume_endpoint():
    ...
    try:
        if not all(k in request.form for k in ['jobData', 'cvText', 'atsScore']):
            ...
            return jsonify({"success": False, "error": "Missing job data, CV text, or ATS score."}), 400
        ...
        if 'kbFiles' in request.files:
            ...
            for file_storage in files:
                if file_storage and allowed_file(file_storage.filename):
                    filepath = save_uploaded_file(file_storage, Config.UPLOAD_FOLDER, f"kb_{int(time.time())}_{secure_filename(file_storage.filename)}")
        ...
        try:
            ...
            generated_markdown_resume, cloudconvert_api_key = run_joblo(cv_text, job_data, kb_data_chunks)
            ...
            process_resume(generated_markdown_resume, cloudconvert_api_key, output_docx_path) # Saves as output_docx_path
            ...
            prompt_improved_ats = prepare_prompt(job_data, embedded_improved_resume, custom_prompt_improved_ats)
            improved_ats_output_str = gpt_generate_resume(openai_api_key, prompt_improved_ats, model="gpt-4o-mini", temperature=0.1) # Lowered temperature
            
            import re
            json_match_improved_ats = re.search(r'```json\\s*(.*?)\\s*```|{.*}', improved_ats_output_str, re.DOTALL)
            improved_ats_score_data = {}

            if json_match_improved_ats:
                json_str_improved_ats = json_match_improved_ats.group(1) or json_match_improved_ats.group(0)
                json_str_improved_ats = json_str_improved_ats.replace('```json', '').replace('```', '').strip()
                try:
                    improved_ats_score_data = json.loads(json_str_improved_ats)
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse improved ATS JSON from LLM output: {json_str_improved_ats}. Error: {e}")
                    return jsonify({"success": False, "error": "Failed to parse improved ATS score JSON. Invalid format from AI."}), 500
            else:
                logger.error(f"Failed to find improved ATS JSON in LLM output: {improved_ats_output_str}")
                return jsonify({"success": False, "error": "Failed to find improved ATS score JSON in AI response."}), 500
            
            import shutil
            shutil.rmtree(temp_dir, ignore_errors=True)
            
            logger.info("Resume generation and improved ATS analysis completed successfully")
            return jsonify({
                "success": True,
                "data": {
            ...
            return jsonify({"success": False, "error": f"Error during resume core processing: {str(e_inner)}"}), 500
            
    except Exception as e_outer:
        logger.error(f"Error in /generate-resume endpoint: {str(e_outer)}")
        logger.error(traceback.format_exc())
        return jsonify({"success": False, "error": f"Failed to generate improved resume: {str(e_outer)}"}), 500
