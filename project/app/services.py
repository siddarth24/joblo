...
from .utils import (
    ...
    allowed_file,
    save_uploaded_file,
)  # Assuming load_prompt is in project/app/utils.py
from joblo_core import (
    create_embedded_resume,
    prepare_prompt,
    extract_text_and_links_from_file,
)  # Direct imports if needed by services
...
class ProcessingService:
    ...
    def initiate_ats_analysis(job_data_str: str, cv_text: str):
        ...
        try:
            ...
            return {
                "success": False,
                "error": "Invalid JSON format for jobData.",
                "status_code": 400,
            }
        ...
        if not cv_text or not cv_text.strip():
            ...
            return {
                "success": False,
                "error": "cvText cannot be empty.",
                "status_code": 400,
            }

        try:
            embedded_resume = create_embedded_resume(cv_text)
            custom_prompt_str = load_prompt(
                "ats_analysis.txt"
            )  # Assumes load_prompt is accessible
            final_prompt_str = prepare_prompt(
                job_data, embedded_resume, custom_prompt_str
            )
            ...
            logger.info(
                f"ProcessingService: Dispatched ATS analysis task: {task.id} with model {current_app.config['LLM_MODEL_NAME']}"
            )
            ...
            return {
                ...
                "task_id": task.id,
                "status_url": url_for(
                    "processing.get_task_status", task_id=task.id, _external=True
                ),  # Note: url_for needs app context
                "status_code": 202,
            }
        except Exception as e:
            logger.error(
                f"ProcessingService: Error during ATS analysis initiation: {str(e)}",
                exc_info=True,
            )
            return {
                "success": False,
                "error": f"Failed to initiate ATS analysis: {str(e)}",
                "status_code": 500,
            }

    @staticmethod
    def initiate_resume_generation_workflow(
        job_url,
        job_description_str,
        base_resume_text_form,
        resume_file_storage,
        kb_file_storages,
        ...
        temp_files_managed_by_service = (
            []
        )  # For files created and used solely within this service call

        try:
            upload_folder = current_app.config["UPLOAD_FOLDER"]
            if not current_app.config.get(
                "OPENAI_API_KEY"
            ) or not current_app.config.get("CLOUDCONVERT_API_KEY"):
                logger.error(
                    "ProcessingService: Missing OpenAI or CloudConvert API Key."
                )
                return {
                    "success": False,
                    "error": "Configuration error: Missing API Keys.",
                    "status_code": 503,
                }
            ...
            if resume_file_storage:
                if not resume_file_storage.filename or not allowed_file(
                    resume_file_storage.filename
                ):
                    return {
                        "success": False,
                        "error": "Invalid resume file type for resumeFile.",
                        "status_code": 400,
                    }

                s_resume_filename = os.path.basename(
                    secure_filename(resume_file_storage.filename)
                )
                temp_resume_path = save_uploaded_file(
                    resume_file_storage,
                    upload_folder,
                    f"service_temp_resume_{int(time.time())}_{s_resume_filename}",
                )
                ...
                if not base_resume_text or not base_resume_text.strip():
                    return {
                        "success": False,
                        "error": "Uploaded resume file is empty or text extraction failed.",
                        "status_code": 400,
                    }
            elif not base_resume_text_form or not base_resume_text_form.strip():
                return {
                    "success": False,
                    "error": "Provided baseResumeText is empty.",
                    "status_code": 400,
                }
            ...
            if job_url and (
                not actual_job_data or not actual_job_data.get("Description")
            ):
                scrape_init_task = async_adaptive_scraper.delay(job_url)
                logger.info(
                    f"ProcessingService: Dispatched job scraping task {scrape_init_task.id} for resume generation."
                )
                return {
                    "success": True,
                    ...
                    "scrape_task_status_url": url_for(
                        "processing.get_task_status",
                        task_id=scrape_init_task.id,
                        _external=True,
                    ),
                    "intermediate_data": {
                ...
                return {
                    "success": False,
                    "error": "Could not obtain a usable job description.",
                    "status_code": 400,
                }
            ...
            for kb_file in kb_file_storages:
                if kb_file and kb_file.filename and allowed_file(kb_file.filename):
                    ...
                    kb_path = save_uploaded_file(
                        kb_file,
                        upload_folder,
                        f"service_temp_kb_{int(time.time())}_{s_kb_filename}",
                    )
                    temp_files_managed_by_service.append(kb_path)
                    kb_file_paths_for_rag.append(kb_path)

            relevant_chunks = []
            if kb_file_paths_for_rag and current_app.config.get(
                "ENABLE_RAG_FEATURE", True
            ):
                try:
                    relevant_chunks = extract_relevant_chunks(
                        file_paths=kb_file_paths_for_rag, job_data=actual_job_data
                    )
                except Exception as e_rag:
                    logger.warning(
                        f"ProcessingService: Error extracting RAG chunks: {e_rag}",
                        exc_info=True,
                    )

            embedded_resume = create_embedded_resume(base_resume_text)
            custom_prompt_resume_str = load_prompt("resume_generation.txt")
            final_resume_gen_prompt_str = prepare_prompt(
                actual_job_data,
                embedded_resume,
                custom_prompt_resume_str,
                relevant_chunks,
            )
            ...
            logger.info(
                f"ProcessingService: Dispatched resume generation chain. Final task ID: {chain_result.id}"
            )
            ...
            logger.error(
                f"ProcessingService: Error in resume generation workflow: {str(e)}",
                exc_info=True,
            )
            return {
                "success": False,
                "error": f"An unexpected server error occurred in service: {str(e)}",
                "status_code": 500,
            }
        finally:
            ...
            for f_path in temp_files_managed_by_service:
                if os.path.exists(f_path):
                    try:
                        os.remove(f_path)
                        logger.info(
                            f"ProcessingService: Cleaned temp service file: {f_path}"
                        )
                    except Exception as e_clean:
                        logger.error(
                            f"ProcessingService: Error cleaning temp service file {f_path}: {e_clean}"
                        )

    @staticmethod
    def initiate_docx_conversion(
        markdown_content: str | None,
        input_md_path_param: str | None,
        ...
        temp_md_file_created_by_service = (
            None  # Path to MD file if created by this service
        )
        ...
        try:
            ...
            if not current_app.config.get("CLOUDCONVERT_API_KEY"):
                ...
                return {
                    "success": False,
                    "error": "Configuration error: Missing CloudConvert API Key.",
                    "status_code": 503,
                }
            ...
            if input_md_path_param:
                ...
                if not abs_input_path.startswith(abs_upload_folder):
                    logger.warning(
                        f"ProcessingService: Attempt to access unsafe path for MD conversion: {input_md_path_param}"
                    )
                    return {
                        "success": False,
                        "error": "Invalid input file path for Markdown.",
                        "status_code": 400,
                    }
                if not os.path.exists(abs_input_path):
                    return {
                        "success": False,
                        "error": f"Provided inputMarkdownFilePath does not exist: {input_md_path_param}",
                        "status_code": 400,
                    }
                md_path_for_task = abs_input_path
            elif markdown_content:
                temp_md_filename = (
                    f"service_temp_md_for_conversion_{int(time.time())}.md"
                )
                # Ensure filename is secure before joining
                temp_md_file_created_by_service = os.path.join(
                    upload_folder, secure_filename(temp_md_filename)
                )
                try:
                    with open(
                        temp_md_file_created_by_service, "w", encoding="utf-8"
                    ) as f_md:
                        f_md.write(markdown_content)
                    logger.info(
                        f"ProcessingService: Markdown content saved to temporary file: {temp_md_file_created_by_service}"
                    )
                    md_path_for_task = temp_md_file_created_by_service
                    delete_temp_md_on_success = (
                        True  # This file was created by the service for the task
                    )
                except Exception as e_save:
                    logger.error(
                        f"ProcessingService: Failed to save temporary MD file: {e_save}",
                        exc_info=True,
                    )
                    return {
                        "success": False,
                        "error": "Failed to prepare Markdown file for conversion.",
                        "status_code": 500,
                    }
            ...
            if not md_path_for_task:
                return {
                    "success": False,
                    "error": "Could not determine Markdown input for conversion.",
                    "status_code": 500,
            ...
            logger.info(
                f"ProcessingService: Dispatched DOCX conversion task: {convert_task.id} for input {md_path_for_task}"
            )
            ...
            return {
                ...
                "docx_conversion_task_status_url": url_for(
                    "processing.get_task_status",
            ...
            logger.error(
                f"ProcessingService: Error in DOCX conversion: {str(e)}", exc_info=True
            )
            return {
                "success": False,
                "error": f"An unexpected server error occurred in service: {str(e)}",
            ...
            if (
                temp_md_file_created_by_service
                and not delete_temp_md_on_success
                and os.path.exists(temp_md_file_created_by_service)
            ):
                try:
                    os.remove(temp_md_file_created_by_service)
                    logger.info(
                        f"ProcessingService: Cleaned service-created temp MD file (fallback): {temp_md_file_created_by_service}"
                    )
                except Exception as e_clean_fallback:
                    logger.error(
                        f"ProcessingService: Error cleaning service-created temp MD file (fallback) {temp_md_file_created_by_service}: {e_clean_fallback}"
                    )

    @staticmethod
    def initiate_job_application_processing(
        job_url: str | None, job_description_form: str | None, resume_file_storage
    ):
        """
        ...
        try:
            ...
            if not allowed_file(resume_file_storage.filename):
                return {
                    "success": False,
                    "error": "Valid resume file type required.",
                    "status_code": 400,
                }
            ...
            temp_resume_path_service = save_uploaded_file(
                resume_file_storage,
                upload_folder,
                f"service_resume_initial_{int(time.time())}_{s_filename}",
            )
            ...
            if not cv_text or not cv_text.strip():
                ...
                return {
                    "success": False,
                    "error": "Extracted text from resume is empty.",
                    "status_code": 400,
                }
            ...
            if job_url:
                ...
                logger.info(
                    f"ProcessingService: Dispatched job scraping task: {scrape_task.id} for URL: {job_url}"
                )
                ...
                try:
                    job_data_for_task = (
                        json.loads(job_description_form)
                        if job_description_form.strip().startswith("{")
                        else {"Description": job_description_form}
                    )
                except json.JSONDecodeError:
                ...
                final_ats_prompt_str = prepare_prompt(
                    job_data_for_task, embedded_resume, custom_prompt_ats_str
                )
                ...
                logger.info(
                    f"ProcessingService: Dispatched ATS analysis task: {ats_task.id} with model {current_app.config['LLM_MODEL_NAME']}"
                )

                s_company = secure_filename(job_data_for_task.get("Company", "Company"))
                s_job_title = secure_filename(
                    job_data_for_task.get("Job Title", "Position")
                )
                ...
                if os.path.exists(temp_resume_path_service):
                    try:
                        os.remove(temp_resume_path_service)
                        logger.info(
                            f"ProcessingService: Cleaned temp resume file: {temp_resume_path_service} after direct ATS dispatch."
                        )
                    except Exception as e_clean:
                        logger.error(
                            f"ProcessingService: Error cleaning temp resume file {temp_resume_path_service}: {e_clean}"
                        )
                ...
                if os.path.exists(temp_resume_path_service):
                    os.remove(temp_resume_path_service)  # Cleanup attempt
                return {
                    "success": False,
                    "error": "Could not determine job data for processing.",
                    "status_code": 400,
                }

        except Exception as e:
            logger.error(
                f"ProcessingService: Error in job application processing: {str(e)}",
                exc_info=True,
            )
            ...
            if temp_resume_path_service and os.path.exists(temp_resume_path_service):
                try:
                    ...
                    logger.error(
                        f"ProcessingService: Error cleaning temp resume (fallback): {e_clean_fallback}"
                    )
            return {
                "success": False,
                "error": f"An unexpected server error occurred in service: {str(e)}",
