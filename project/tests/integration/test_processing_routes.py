...
def app():
    ...
    upload_folder = _app.config.get(
        "UPLOAD_FOLDER", "/tmp/joblo_test_uploads_integration"
    )
...
def mock_service_process_job_application(mock_celery_task):
    with patch(
        "project.app.services.ProcessingService.process_job_application"
    ) as mock_method:
...
def mock_service_analyze_ats(mock_celery_task):
    with patch(
        "project.app.services.ProcessingService.analyze_ats_resume_job_desc"
    ) as mock_method:
        mock_method.return_value = {
            "task_id": mock_celery_task.id,
            "status": "pending",
...
def mock_service_generate_resume(mock_celery_task):
    with patch(
        "project.app.services.ProcessingService.generate_custom_resume"
    ) as mock_method:
        # Simulate the new chained task structure return value
        mock_method.return_value = {
            "final_task_id": mock_celery_task.id,
            "status": "pending",
            "message": "Resume generation and conversion chain started",
        }
        yield mock_method
...
def mock_service_convert_md_to_docx(mock_celery_task):
    with patch(
        "project.app.services.ProcessingService.convert_markdown_to_docx"
    ) as mock_method:
        mock_method.return_value = {
            "task_id": mock_celery_task.id,
            "status": "pending",
            "message": "Markdown to DOCX conversion started",
        }
        yield mock_method
...
def test_process_job_application_success(
    client, app, mock_service_process_job_application
):
    data = {
        "job_description_url": "http://example.com/job/123",
        "custom_prompt_text": "Make it good for a FAANG.",
        "knowledge_base_files_info": json.dumps(
            [{"filename": "kb1.txt", "content": "kb data"}]
        ),
    ...
    with open(resume_file_path, "rb") as resume_fp:
        ...
        response = client.post(
            "/processing/process-job-application",
            data=data,
...
def test_process_job_application_missing_resume(
    client, mock_service_process_job_application
):
    data = {
        "job_description_url": "http://example.com/job/123",
    }
...
def test_analyze_ats_success(client, app, mock_service_analyze_ats):
    ...
    with open(resume_file_path, "rb") as resume_fp:
        data["resume_file"] = (resume_fp, "fake_resume.pdf")
        response = client.post(
            "/processing/analyze-ats", data=data, content_type="multipart/form-data"
        )
...
def test_generate_resume_success(client, app, mock_service_generate_resume):
    ...
    with open(resume_file_path, "rb") as resume_fp:
        data["resume_file"] = (resume_fp, "fake_resume.pdf")
        response = client.post(
            "/processing/generate-resume", data=data, content_type="multipart/form-data"
        )
...
def test_convert_md_to_docx_success(client, app, mock_service_convert_md_to_docx):
    ...
    data = {"delete_input_on_success": "true"}
    with open(md_file_path, "rb") as md_fp:
        data["markdown_file"] = (md_fp, "fake_markdown.md")
        response = client.post(
            "/processing/convert-to-docx", data=data, content_type="multipart/form-data"
        )
...
def test_convert_md_to_docx_missing_file(client, mock_service_convert_md_to_docx):
    response = client.post(
        "/processing/convert-to-docx", data={}, content_type="multipart/form-data"
    )
    assert response.status_code == 400
    json_response = response.get_json()
    assert not json_response["success"]
...
@patch(
    "project.app.routes.celery_current_app.AsyncResult"
)  # Patch where AsyncResult is used
...
def test_get_task_status_success_with_result(mock_async_result, client):
    ...
    mock_result_instance.info = {
        "output_file": "/path/to/result.docx",
        "original_filename": "resume.docx",
    }
    mock_result_instance.result = (
        mock_result_instance.info
    )  # For SUCCESS, .result often holds .info
    mock_async_result.return_value = mock_result_instance
...
def test_get_task_status_success_with_direct_result(mock_async_result, client):
    ...
    mock_result_instance.info = (
        mock_result_instance.result
    )  # Often .info mirrors .result for simple cases
    mock_async_result.return_value = mock_result_instance
...
def mock_service_process_job_application_raises_value_error(mock_celery_task):
    with patch(
        ...
        mock_method.side_effect = ValueError(
            "Service validation failed: Invalid job URL"
        )
        yield mock_method


def test_process_job_application_service_value_error(
    client, app, mock_service_process_job_application_raises_value_error
):
    data = {
    ...
    )  # Expecting the route to catch ValueError and return 400
    json_response = response.get_json()
    assert not json_response["success"]
